...
export default function StudioMatchCard({
  ...
  const isRemovedUserMatch = status === 'deleted_user';
  const statusMeta = useMemo(() => {
    ...
    if (status === 'deleted_user') return { label: t('studio.match.status.deleted_user'), cls: 'bg-rose-50 text-rose-800 border-rose-200' };
  ...
  const ageText = typeof other?.age === 'number' ? `${other.age} ${t('studio.common.ageSuffix')}` : '';
  ...
  const aboutFull = useAutoLocalizedProfileText(other, 'about', i18n.language) || getLocalizedProfileText(other, 'about', i18n.language) || safeStr(other?.about || other?.details?.about || other?.bio || other?.details?.bio);
  const expectationsFull = useAutoLocalizedProfileText(other, 'expectations', i18n.language) || getLocalizedProfileText(other, 'expectations', i18n.language) || safeStr(other?.expectations || other?.details?.expectations);
  ...
  const setMyPhotoAccessForThisMatch = async (next) => {
    if (interactionsDisabled || isRemovedUserMatch) {
      blockInteraction();
      return;
    }
  ...
  const openProfileDetails = () => {
    if (isRemovedUserMatch) return;
    if (!match?.id) return;
    navigate(`/app/match/${match.id}?openProfile=1&profileTab=details`, { state: { openProfile: true, profileTab: 'details' } });
  };
  ...
  return (
    <>
    <div className="relative flex flex-col overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-900 shadow-sm transition-all hover:shadow-md">
      <div>
        <div className="relative">
          ...
          {!OPEN_CHAT_MODEL && status === 'mutual_interest' && mutualLiked && otherStartedActive && !iStartedActive ? (
          ...
          <div className="relative aspect-square w-full overflow-hidden bg-slate-100">
            ...
            {!OPEN_CHAT_MODEL && status === 'mutual_interest' && mutualLiked ? (
          ...
          {!canSeeOtherPhotos ? (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center p-4">
              <div className="max-w-[18rem] rounded-2xl bg-black/65 px-4 py-2.5 text-center text-[11px] font-semibold leading-5 text-white">
                {photoBlockedByReciprocity
                  ? t('studio.match.photos.reciprocityBlocked')
                  : photoBlockedByOtherPrivacy
                    ? t('studio.match.photos.hiddenByOwner')
        ...
        <div className="p-4 flex-1 text-slate-900">
          ...
          {otherUserCode ? (
            <div className="mt-1">
              ...
              {presenceUiEnabled && presenceMeta.label ? (
        ...
        <div className="p-4 pt-0 flex flex-col gap-2">
          {isRemovedUserMatch ? (
            <div className="rounded-md border border-rose-200 bg-rose-50 p-3 text-xs font-semibold text-rose-900">
          ...
          {photoBlockedByOtherPrivacy && !photoBlockedByReciprocity ? (
            <button
              ...
              disabled={photoRequestState.loading || photoRequestState.status === 'pending' || photoRequestState.status === 'approved'}
          ...
          {myPhotosBlurred ? (
            <button
              ...
              disabled={photoAccessState.loading}
              className={
            ...
            {incomingLikeNeedsDecision ? (
              <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    data-testid={match?.id ? `match-card-profile-details-${match.id}` : 'match-card-profile-details'}
          ...
          {!OPEN_CHAT_MODEL && longChatAllowedHere && !activeChatNoticeSeen ? (
