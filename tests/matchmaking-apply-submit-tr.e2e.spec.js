...
function buildRunScopedEmail(token) {
  return `seed.apply.tr.${token}@example.test`;
}

function buildRunScopedUsername(token) {
  return `applytr${token.slice(-8)}`;
}
...
test('TR user can fill matchmaking apply form and submit successfully', async ({ page }) => {
  ...
  console.log(`[e2e] apply-submit-tr email: ${email}`);
  console.log(`[e2e] apply-submit-tr username: ${username}`);
  ...
  try {
    ...
    await dismissBlockingDialogs(page);
    await form.getByRole('button', { name: /^fotoğraf$|^fotograf$|^foto$|^photo$/i }).click();
    const photoDialog = page.getByRole('dialog', { name: /fotoğraf|foto|photo/i });
    await expect(photoDialog).toBeVisible();
    const applyPhotoChooserPromise = page.waitForEvent('filechooser');
    await photoDialog.locator('label[for="mk-photo1"]').first().click();
    const applyPhotoChooser = await applyPhotoChooserPromise;
    expect(applyPhotoChooser.isMultiple()).toBe(false);
    await applyPhotoChooser.setFiles({
      name: 'e2e-photo.png',
      mimeType: 'image/png',
      buffer: buildTinyPngBuffer(),
    });
    await photoDialog.getByRole('button', { name: /selesai|tamamla|tamam|done/i }).click();
    ...
    await form.getByRole('button', { name: /selesai|bitir|finish|tamam/i }).click();
    ...
    console.log(`[e2e] apply-submit-tr final url: ${page.url()}`);
    ...
    expect(String(savedData?.details?.occupation || '')).toBe('Ogretmen');
